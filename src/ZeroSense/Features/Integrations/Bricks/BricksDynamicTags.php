...
class BricksDynamicTags implements FeatureInterface
{
        ...
        foreach ($recipeGroups as $recipeId => $group) {
            ...
            if (is_array($recipeIngredients)) {
                foreach ($recipeIngredients as $ingRow) {
                    ...
                    $html .= '<div class="brxe-div fdr-card__field"><span class="brxe-text-basic fdr-card__field-label">' . esc_html($ingName) . '</span><span class="brxe-text-basic fdr-card__field-value" style="display:flex;gap:4px;align-items:baseline"><span style="font-variant-numeric:tabular-nums">' . esc_html($this->formatNumber($normalized['qty'])) . '</span><span style="min-width:2.5em">' . esc_html($normalized['unit']) . '</span></span></div>';
                }
            }
    ...
    private function getRecipeTotalIngredientsSimple($post): string
    {
        ...
        foreach ($totals as $t) {
            ...
            $html .= '<span class="brxe-text-basic fdr-card__field-value" style="display:flex;gap:4px;align-items:baseline"><span style="font-variant-numeric:tabular-nums">' . esc_html($this->formatNumber($normalized['qty'])) . '</span><span style="min-width:2.5em">' . esc_html($normalized['unit']) . '</span></span>';
            $html .= '</div>';
        }
        ...
        foreach ($recipeGroups as $recipeId => $group) {
            ...
            if (is_array($recipeLiquids)) {
                foreach ($recipeLiquids as $liqRow) {
                    ...
                    if ($liqName === '') continue;
                    $html .= '<div class="brxe-div fdr-card__field"><span class="brxe-text-basic fdr-card__field-label">' . esc_html($liqName) . '</span><span class="brxe-text-basic fdr-card__field-value" style="display:flex;gap:4px;align-items:baseline"><span style="font-variant-numeric:tabular-nums">' . esc_html($this->formatNumber($amount)) . '</span><span style="min-width:2.5em">l</span></span></div>';
                }
            }
    ...
    private function getRecipeUtensilsSimple($post): string
    {
        ...
        foreach ($totals as $t) {
            ...
            $html .= '<div class="brxe-div fdr-card__field">';
            $html .= '<span class="brxe-text-basic fdr-card__field-label">' . esc_html($termName) . '</span>';
            $html .= '<span class="brxe-text-basic fdr-card__field-value" style="display:flex;gap:4px;align-items:baseline"><span style="font-variant-numeric:tabular-nums">' . esc_html($this->formatNumber($qty)) . '</span><span style="min-width:2.5em">' . esc_html($unit) . '</span></span>';
            $html .= '</div>';
        }
    ...
    private function getRecipeLiquidsSimple($post): string
    {
        ...
        foreach ($totals as $t) {
            ...
            $html .= '<div class="brxe-div fdr-card__field">';
            $html .= '<span class="brxe-text-basic fdr-card__field-label">' . esc_html($termName) . '</span>';
            $html .= '<span class="brxe-text-basic fdr-card__field-value" style="display:flex;gap:4px;align-items:baseline"><span style="font-variant-numeric:tabular-nums">' . esc_html($this->formatNumber($qty)) . '</span><span style="min-width:2.5em">l</span></span>';
            $html .= '</div>';
        }
