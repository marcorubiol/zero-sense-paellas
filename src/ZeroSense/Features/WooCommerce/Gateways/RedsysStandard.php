            ...
            if ($isSuccess) {
                ...
                if ($order->has_status('deposit-paid')) {
                    ...
                    if ($intent === 'remaining' && $isBalancePayment) {
                        ...
                        if ($logger) {
                            $logger->info(sprintf('Balance payment detected: order=%d amount=%.2f remaining=%.2f', $orderId, $amountRaw, $remainingAmount), ['source' => 'zero-sense-redsys-standard']);
                        }
                        ...
                        if ($logger) {
                            $logger->warning(sprintf('Balance payment amount mismatch: order=%d expected=%.2f received=%.2f', $orderId, $remainingAmount, $amountRaw), ['source' => 'zero-sense-redsys-standard']);
                        }
                        ...
                        if ($logger) {
                            $logger->info(sprintf('Balance payment inferred from amount: order=%d amount=%.2f', $orderId, $amountRaw), ['source' => 'zero-sense-redsys-standard']);
                        }
                        $intent = 'balance';
                        ...
                        if ($logger) {
                            $logger->warning(sprintf('Duplicate deposit payment ignored: order=%d amount=%.2f', $orderId, $amountRaw), ['source' => 'zero-sense-redsys-standard']);
                        }
                        status_header(200); exit;
                    } else {
